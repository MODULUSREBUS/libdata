...
async fn timeout_no_connection() -> Result<()>
{
    ...
    assert!(matches!(Pin::new(&mut proto_a).poll_next(&mut cx), Poll::Pending));
    sleep(Duration::from_millis(60)).await;
    assert!(matches!(Pin::new(&mut proto_a).poll_next(&mut cx), Poll::Pending));
    sleep(Duration::from_millis(keepalive_ms)).await;
    assert!(matches!(Pin::new(&mut proto_a).poll_next(&mut cx), Poll::Ready(Some(Err(_)))));
...
async fn timeout_reading_resets_timeout_writing_not() -> Result<()> {
    ...
    assert!(matches!(Pin::new(&mut proto_a).poll_next(&mut cx), Poll::Pending));
    assert!(matches!(Pin::new(&mut proto_b).poll_next(&mut cx), Poll::Ready(Some(
    ...
    sleep(Duration::from_millis(60)).await;
    assert!(matches!(Pin::new(&mut proto_a).poll_next(&mut cx), Poll::Ready(Some(Err(_)))));
    assert!(matches!(Pin::new(&mut proto_b).poll_next(&mut cx), Poll::Pending));

    sleep(Duration::from_millis(60)).await;
    assert!(matches!(Pin::new(&mut proto_b).poll_next(&mut cx), Poll::Ready(Some(Err(_)))));
